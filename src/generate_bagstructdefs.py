...
# main2() is a testing run config, please ignore
# keeping it in as script is still in development
# so still useful for me to be able to test with hard coded
# edge case examples 
def main2():
    # a nice edge case-y example
   
    def addArraySuffixes(tupleList, arraySize : str):
        listWithSuffixes = []
        if arraySize == '':
            return tupleList
        else:
            for i in range(int(arraySize)):
                ...
                for c in tupleList:
                    fieldType = c[0]
                    fieldName = c[1]
                    newFieldType = processArrayNotation(fieldType)[0]
                    newFieldName = fieldName + '_' + str(i+1)
                    newTuple = (newFieldType, newFieldName)
                    iList.append(newTuple)
                listWithSuffixes.extend(iList)
    ...
    def addRelativePath(fieldType, msgDir):
        potentialRelPath = os.path.join(os.path.expanduser('~'), '/opt', 'ros', 'noetic', 'share', msgDir, 'msg', fieldType + '.msg')
        if os.path.exists(potentialRelPath):
            return os.path.join(msgDir, fieldType)
        else:
    ...
    # returns a tuple where tuple[0] is the field type without any array digits
    # and tuple[1] is a string reprsenting the array size, or '' if there was no array
    def processArrayNotation(field: str):
        pattern = r'^(.*)\[(\d+)\]$'
        match = re.match(pattern, field)
        if match:
            return (match.group(1), match.group(2))
    ...
    # takes in ... and returns absolute path if path can be found
    # with supplied parameters, false no path can be found 
    def createPath(miniPath):
        rootDir = os.path.expanduser('~')
        name = 'jason'
        ...
        indvSecondPath = os.path.basename(miniPath)
        indvSecondPathNoDigits = processArrayNotation(indvSecondPath)[0]

        msgPathCustom = os.path.join(rootDir, 'ros', f'{name}_ws', 'src', 'ds_msgs', indvFirstDir, 'msg', indvSecondPathNoDigits + '.msg')
        msgPathCommon = os.path.join(rootDir, '/opt', 'ros', 'noetic', 'share', indvFirstDir, 'msg', indvSecondPathNoDigits + '.msg')
        #msgPathRelative = os.path.join(rootDir, '/opt', 'ros', 'noetic', 'share', indvFirstDirRelative, 'msg', indvSecondPathNoDigits + '.msg')
        if os.path.exists(msgPathCustom):
            return msgPathCustom
        elif os.path.exists(msgPathCommon):
            return msgPathCommon
    ...
    def processIndvTuple(indvTuple):
        # 'cur' prefix indicates variable relating to tuple being processed, indvTuple 
        pattern = r'^\s*([\w/\[\]]+)\s+([\w/\[\]]+)'
        curContainer = []
        # includes digits if they are there 
        curMiniPath = indvTuple[0]
        curName = indvTuple[1]
        relDir = '/'.join(curMiniPath.rsplit('/', 1)[:-1])
        # will not include digits 
        curAbsPath = createPath(curMiniPath)
        curArrayDigitString = processArrayNotation(curMiniPath)[1]
        ...
        if bool(curAbsPath):
            with open(curAbsPath, 'r') as infile:
                for line in infile:
                    ...
                    # now living in the subfields of curMiniPath 
                    if not line.startswith('#') and not line.isspace():
                        line = re.sub(r'#.*', '', line)
                        line = re.sub(r'.*=', '', line)
                        match = re.match(pattern, line)
                        if match:
                            ...
                            if match.group(1) == 'Header':
                                ...
                                fieldType = match.group(1)
                            fieldType = addRelativePath(fieldType, relDir)
                            field = match.group(2)
                            # create the tuple 
                            lineTuple = (fieldType, curName + '.' + field)
                            lineContainer.append(lineTuple)
        ...
        curSuffixedContainer = []
        for container in curContainer:
            suffixedContainer = addArraySuffixes(container, curArrayDigitString)
            curSuffixedContainer.append(suffixedContainer)
        ...
        def recurOnCurContainer(curSufContainer):
            ...
            for item in curSufContainer:
                if isinstance(item, list):
                    processed_item = recurOnCurContainer(item)  # Recur on nested lists
                    processed_structure.append(processed_item)
                elif isinstance(item, tuple):
                    # addRelPath spec allows this to be valid path indicator
                    if '/' in item[0] and '[]' not in item[0] or processArrayNotation(item[0])[1] != '':
                        processedIndvTuple = processIndvTuple(item) 
                        processed_structure.append(processedIndvTuple)
                    else:
                        processed_structure.append(item)
            return processed_structure
    ...
    # ultimately emit one structTupleListofLists per struct.yaml
    def processStructTuples(structTuplesList):
        # say struct tuple five times fast 
        structTupleListofLists = []
        for structTuple in structTuplesList:
            structTupleListofLists.append(processIndvTuple(structTuple))     
        return structTupleListofLists


    
    # generates a dictionary where every key is
    # struct.yaml and every value is a list of lists of tuples with
    # path types unwrapped. 
    ...
    # overallDirRel = nav_msgs
    # fieldDirRel = geometry_msgs


    keyPath = 'path.yaml'
    value0Path= 'nav_msgs/Path'
    value1Path =  [('std_msgs/Header', 'header'),
              ('geometry_msgs/Vector3[]', 'velocity')]
    value1Path_1 = [('std_msgs/Header[]', 'header')]
    
    keyOdom = 'odometry.yaml'
    value0Odom= 'nav_msgs/Odometry'
    value1Odom = [('ds_core_msgs/DsHeader', 'ds_header'),
             ('uint8[]' , 'child_frame_id'), ('geometry_msgs/PoseWithCovariance[3]' , 'pose')]
    
    structFieldDict = {}

    keyHeader = 'dsheader.yaml'
    value0dsheader = 'ds_core_msgs/DsHeader'
    value1dsheader = [('ds_core_msgs/DsHeader', 'ds_header')]

    """ ('std_msgs/Header', 'header'),
             ('string[4]' , 'child_frame_id'), """
    structFieldDict[keyPath] = (value0Path, value1Path)
    structFieldDict[keyOdom] = (value0Odom, value1Odom)
    #structFieldDict[keyPath] = (value0Path, value1Path_1)
    
    #structFieldDict[keyHeader] = (value0dsheader, value1dsheader)


    def generate_StructFieldsUnwrappedDict():
        unwrappedDict = {}
        for key, value in structFieldDict.items():
            unwrappedDict[key] = processStructTuples(value[1])
        return unwrappedDict
    
    testDict = generate_StructFieldsUnwrappedDict()
    print('final dict info')
    for key, value in testDict.items():
        print(key)
        print(len(value))
        print(value)

    def populate_structs(unwrappedDict):
        vehName = ''
        originalUnwrappedDict = unwrappedDict
        originalStructList = list(originalUnwrappedDict.keys())
        ...
        def handleSpecialTypes(processedStructList, originalDictionary):
            ...
            # fieldtype, field for an entire struct, and creates a new
            # nested list with modificiations that account for special types 
            def processValue(nestedTupleList):
                processedValue = []
                ...
                for item in nestedTupleList:
                    ...
                    # append tuples to processedValue accordingly
                        ...
                        # bool or string type indicates potential special type
                            ...
                            newType1 = f"Identified a possible special type in {item})."
                            newType2 = f"Please enter a new type for '{item[0]}' (e.g., 'cell' or 'pwr_state')"
                            newType3 = f"or press Enter if '{item[0]}' is an acceptable type for '{item[1]}': "
                            newType = input(newType1 + '\n' + newType2 + '\n' + newType3)
                            if newType != '':
            ...
            for key, value in originalDictionary.items():
                if key in processedStructList:
                    processedUnwrappedDict[key] = processValue(value)
            return processedUnwrappedDict
           

        processeedUnwrappedDict = handleSpecialTypes(processedStructList, originalUnwrappedDict)
        print('processedDict is ')
        for key, value in processeedUnwrappedDict.items():
            print(key)
            [print(v) for v in value]
    
        
        # actually modifies content of /struct_defs
        def populateStructDefFiles():
            pass



    populate_structs(testDict)
